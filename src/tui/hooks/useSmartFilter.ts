...
export function useSmartFilter(): UseSmartFilterResult {
  ...
  const submitFilter = useCallback(async (description: string, allEmails: Email[]) => {
    ...
    console.error('[useSmartFilter] Starting filter:', trimmedDesc, 'with', allEmails.length, 'emails');
    ...
    setFilterError(null);
    setProgress({ evaluatedCount: 0, totalCount: allEmails.length, currentBatch: 0, totalBatches: 0 });
    ...
    try {
      ...
      const result = await runSmartFilter({
        ...
        onProgress: (prog) => {
          console.error('[useSmartFilter] Progress:', prog.evaluatedCount, '/', prog.totalCount, 'batch', prog.currentBatch, '/', prog.totalBatches);
          setProgress({
