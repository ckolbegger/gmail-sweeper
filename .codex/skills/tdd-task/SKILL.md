---
name: tdd-task
description: Implement a single task using strict serial TDD workflow with full-suite validation, progress tracking, and atomic commits.
...
**⚠️ IMPORTANT:** This skill can be run directly by Codex or dispatched by an orchestrator agent. Use one invocation per task.
...
```
User: "Implement US1"
Orchestrator/Main Agent: Dispatches one task at a time with this skill
  "Use tdd-task to implement T001, then T002 ... in strict sequence"
Task Agent: Completes one task and returns status
Orchestrator/Main Agent: Proceeds to next task and reports summary to user
```
...
```text
Use the tdd-task skill to implement T006
Use the tdd-task skill to implement T006 with --progress-file specs/002-smart-email-filter/tasks.md
Use the tdd-task skill to implement T006 with --max-attempts 5
Use the tdd-task skill to implement T006 with --progress-file docs/sprint-plan.md --max-attempts 15
```

## Parameters

- **task_id** (required): Task identifier to implement (e.g., T006, T015a, TASK-123)
- **--progress-file** (optional): File to update with completion status. If omitted, auto-detect in this order:
  1. `tasks.md` in repo root
  2. If exactly one match exists, `specs/*/tasks.md`
  3. If multiple matches exist, stop and require explicit `--progress-file`
...
- Resolve `progress-file` (explicit path or auto-detect rules above)
- Read the resolved `progress-file` to locate task definition
...
**On Success:**
Return structured summary to the caller (main agent/orchestrator):
...
- <progress-file> (marked T006 complete)
...
- Then rerun tdd-task for T006 to retry
...
```
❌ Error: Task T999 not found in <progress-file>
...
2. Progress file is correct (use --progress-file to override auto-detection)
```

### Scenario: Task already complete
```
⚠️  Warning: Task T006 already marked complete in <progress-file>
...
```text
Use tdd-task to implement T007
```
...
```text
Use tdd-task to implement T011
```
...
```text
Use tdd-task to implement FEATURE-42 with --progress-file docs/sprint-plan.md --max-attempts 5
```
...
- **Context preservation:** This skill is designed to keep main conversation context clean by doing implementation in task-scoped runs
...
```
~/.codex/skills/tdd-task/SKILL.md
```
