...
**Resolution**: `useEmailSummary` now accepts `initialSummary?: EmailSummary` to start directly in `ready` state. Live update while viewing (polling) was evaluated and dropped — it caused the full-detail view to flip back to summary immediately since the summary already existed in cache.

- [x] B001-T001 In `useEmailSummary` hook (`src/tui/hooks/useEmailSummary.ts`), add an `initialSummary?: EmailSummary` option; when provided, initialise state to `{ status: 'ready', summary: initialSummary }` instead of `'idle'`; write unit tests covering: hook initialises to ready when initialSummary provided, hook initialises to idle when no initialSummary, switching emails clears ready state unless new initialSummary provided

- [~] B001-T002 DROPPED — polling `useEffect` caused full-detail view to immediately flip back to summary (the summary already existed in cache). Live update while viewing is deferred; on-navigation cache check via B001-T001 is sufficient.
