...
- [ ] T017 Add `AiProviderError` class in `src/core/errors/index.ts`
...
- [ ] T022 [P] [US1] Unit tests for `FilterInput` component in `tests/unit/cli/filter-input.test.tsx` — ink-testing-library, test: renders text input, Enter submits value, displays loading indicator during evaluation (FR-004), displays error message on failure
...
Task: "Unit tests for useSmartFilter hook in tests/unit/cli/use-smart-filter.test.ts"
Task: "Unit tests for FilterInput in tests/unit/cli/filter-input.test.tsx"
Task: "Unit tests for EmailList filter mode in tests/unit/cli/email-list-filter.test.tsx"
