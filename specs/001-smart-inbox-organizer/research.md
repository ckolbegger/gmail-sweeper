...
# Additional Research: Technology Decisions

## Natural Language Processing (NLP) Approach

### Executive Summary

**Recommendation: Hybrid Approach with Ollama as Primary**

| Option | Cost | Latency | Privacy | Offline | Recommendation |
|--------|------|---------|---------|---------|----------------|
| **OpenAI API** | $1-8/1K queries | 0.5-1.5s | Low (cloud) | No | Good for max accuracy |
| **Ollama** | $0 | 0.5-2s | High (local) | Yes | **Recommended** |
| **llama.cpp** | $0 | 0.4-1.8s | High (local) | Yes | Best performance |
| **Hybrid** | $0-4/1K queries | 0.01-2s | Medium-High | Partial | Flexible fallback |

### Ollama (Recommended)

**Why:**
- **Privacy**: Email data never leaves the machine
- **Cost**: Zero ongoing API costs
- **Offline**: Works without internet
- **TypeScript**: Excellent SDK via `ollama` npm package
- **Hardware**: 8GB RAM minimum, 16GB+ recommended

**Models to consider:**
- `qwen2.5:7b` - Recommended balance of speed/quality
- `llama3.2` - Fast, good for simple queries
...
const response = await ollama.chat({
  ...
  messages: [{
    role: 'user',
    content: `Classify these emails: ${emailSubjects.join('\n')}`
  }]
});
```
...
1. **Phase 1 (MVP)**: Ollama with `qwen2.5:7b` for all queries
2. **Phase 2**: Add caching layer for repeated queries
3. **Phase 3**: Optional cloud API fallback for complex queries
...
| Library | React Familiarity | Maintenance | Bundle Size | Best For |
|---------|------------------|-------------|-------------|----------|
| **Ink** | Excellent | Active (~336KB) | Modern React devs | **Recommended** |
| Blessed | Poor | Unmaintained | Smaller | Legacy/widget-heavy |
| oclif | N/A | Active | Minimal | CLI commands only |

### Ink Architecture

**Components needed:**
- `ink` - Core React renderer for terminals
- `ink-table` or custom `<Box>` flexbox for email list
- `ink-text-input` or `@inquirer/prompts` for NL query input
- Built-in `useInput`, `useFocus` for keyboard navigation

**Why Ink:**
- React patterns (hooks, components, JSX)
- Active maintenance (v6.0 released June 2025, React 19 support)
- TypeScript-first
- Binary compilation works with Node.js SEA
- Large ecosystem (34.5k GitHub stars)
...
|----------|----------|-------------|-------------|---------|
| keytar | HIGH | Good | **ARCHIVED** | Avoid |
| **@napi-rs/keyring** | HIGH | Excellent | Active | **Recommended** |
| AES-256-GCM file | MEDIUM-HIGH | Excellent | N/A | Good fallback |

### @napi-rs/keyring

**Why:**
- Uses OS-native secure storage (Keychain/Credential Vault/Secret Service)
- Modern Rust-based, no node-gyp compilation issues
- Full TypeScript/ESM support
- Prebuilt binaries via NAPI-RS
...
const entry = new Entry('gmail-sweep', userEmail);
await entry.setPassword(refreshToken);
...
**Recommendation: @yao-pkg/pkg (fork of vercel/pkg)**

| Tool | Status | Node Support | Native Modules | Verdict |
|------|--------|--------------|----------------|---------|
| pkg (Vercel) | **DEPRECATED** | Node 18 only | Yes | Do not use |
| **@yao-pkg/pkg** | Active | Node 20-22 | Yes | **Recommended** |
| nexe | Maintenance mode | Stuck at 20 | External only | Avoid |
| Deno compile | Active | N/A | No better-sqlite3 | Avoid |
| Node.js SEA | Experimental | 20+ | Via extraction | Watch |
...
**Binary sizes:**
- Minimal: ~35-50 MB
- With better-sqlite3: ~45-60 MB
- With compression: ~25-40 MB

**Build command:**
```bash
npx pkg -t node20-linux-x64,node20-macos-x64,node20-win-x64 dist/cli.js
```
...
|-----------|------------|-----------|
| **Language** | TypeScript 5.3+ / Node.js 20+ | Single language for CLI and web |
| **TUI Framework** | Ink (React for CLI) | React familiarity, active maintenance |
| **Gmail API** | `googleapis` npm package | Official, well-documented, TypeScript support |
| **Database** | better-sqlite3 | Fast, local, zero-config |
| **NLP** | Ollama + qwen2.5:7b | Privacy, offline, zero cost |
| **Auth Storage** | @napi-rs/keyring | OS-native secure storage |
| **Binary Build** | @yao-pkg/pkg | Active fork, native module support |
| **Testing** | Vitest | Fast, TypeScript-native, Vite ecosystem |
| **Bundling** | esbuild | Fast, tree-shaking, for binary compilation |

---

## Research Source Files

- Original Gmail API research: `/home/ckolbegger/src/gmail-sweep/worktrees/kimi/research.md`
- OAuth2 storage deep-dive: `/home/ckolbegger/src/gmail-sweep/worktrees/kimi/research/oauth2-token-storage-research.md`
